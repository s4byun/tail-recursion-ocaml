...
let rec assoc (d,k,l) = match l with
  | [] -> d
  | h::t -> let (x,y) = h in
        if(x = k) then y else assoc(d,k,t);;
...
let fixpoint (f,b) = 
  let g x = let fx = f x in (fx, not (fx = x)) in 
    wwhile (g, b);;
...
let rec exprToString e = match e with
  ...
  | RandomOp (e1,e2,e3) -> "Sqrt(" ^ exprToString e1 ^ "*" ^ exprToString e2 ^ "+" ^ exprToString e2 ^ "*" ^ exprToString e3 ^ ")" ^ "/2"
  | Power (e1, e2) -> "(" ^ exprToString e1 ^ "^" ^ exprToString e2 ^ ")"

let _ = exprToString VarX
let _ = exprToString (Average(Times(Sine(VarX),Cosine(VarY)), Sine(Cosine(VarY))))


let sampleExpr1 = Thresh(VarX,VarY,VarX,(Times(Sine(VarX),Cosine(Average(VarX,VarY)))));;

let _ = exprToString sampleExpr1 
let _ = exprToString (RandomOp(VarX, VarY, VarX))
let _ = exprToString (RandomOp(VarX, VarX, VarY))
let _ = exprToString (RandomOp(VarY, VarY, VarX))
(* build functions:
...
let buildRandomOp(e1,e2,e3)	   = RandomOp(e1,e2,e3)
...
let abs x = if x > 0.0 then x else -1.0 *. x
...
let rec eval (e,x,y) = match e with 
  ...
  | RandomOp (e1,e2,e3)  -> sqrt(abs(eval(e1,x,y)*.eval(e2,x,y) +. eval(e2,x,y)*.eval(e3,x,y)))/.2.0
  | Power (e1,e2)        -> abs(eval(e1,x,y))**abs(eval(e2,x,y))
...
let _ = eval (RandomOp(VarX, VarY, VarX), -1.0, 0.5);;
...
let rec build (rand, depth) = match depth with
  ...
  | _ -> let x = rand(0,8) in
        match x with
          ...
          | 7 -> buildRandomOp(build(rand, depth-1), build(rand, depth-1), build(rand, depth-1))
          | _ -> buildPower(build(rand, depth-1), build(rand, depth-1))
